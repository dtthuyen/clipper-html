...
function clipHtml(
      ...
      if (willSimplifyWhiteSpace) {
        ...
        if (shouldStrip(tagStack[tagStack.length - 1]!)) {
          ...
          const insertSpaceBefore =
            i > 0 && !isWhiteSpace(string.charCodeAt(i - 1));
          const insertSpaceAfter = !isWhiteSpace(
            string.charCodeAt(i + nextBlockSize)
          );
  ...
  if (numChars > maxLength) {
    ...
    while (
      ...
      if (
        tagEndIndex === -1 ||
        string.slice(i + 2, tagEndIndex).trim() !== tagName
      ) {
        throw new Error(`Invalid HTML: ${string}`);
      }
...
function clipPlainText(
  string: string,
  maxLength: number,
  options: CommonClipOptions
): string {
